...
def extract(feature_folder,
    ...
    :param file_handler: instance of fh to create files correctly.
    :param queue:
    ...
    segments = [segment_path
                for segment_path
                in sorted(fu.FileHelper.expand_paths(feature_folder))
    ...
    if sample_size is not None:
        if sample_size < len(segments):
            segments = random.sample(segments, sample_size)
        else:
            eeg_logger.error(f'Sample size: {sample_size} is greater than total number of segment {len(segments)}')
            exit(-2)

    if workers > 1:

        eeg_logger.info('Starting process pool')
        pool = multiprocessing.Pool(processes=workers, initializer=worker_init, initargs=[queue])
        ...
        r = [pool.apply_async(worker_function,
                              ...
                              callback=results.append) for segment in segments]

        while len(results) != len(segments):
            status = f'[Status: {round(100*len(results)/len(segments),2)}%] ({len(results)} segments out of {len(segments)})'
            eeg_logger.info(status)
            time.sleep(120)
